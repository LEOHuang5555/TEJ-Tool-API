...
def get_fin_data(table, ticker, columns = [], start = default_start, end = default_end, fin_type = ['A','Q','TTM']):
...
def get_fin_auditor(table, ticker, columns = [], start = default_start, end = default_end, fin_type = ['A','Q','TTM']):
    # transfer fin_ann_date to daily basis
    # 營業日
    days = pd.date_range(start=start, end=end, freq='D')
    days = pd.DataFrame({'all_dates':days})
